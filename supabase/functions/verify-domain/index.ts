...
serve(async (req) => {
  ...
  try {
    ...
    if (hasCorrectCname) {
      ...
      details = `CNAME record correctly pointing to cinitix.com`;
    } else if (hasARecord) {
      verified = true;
      status = 'verified';
      ...
      details = 'The domain exists but is not configured correctly. Please verify your CNAME record points to cinitix.com';
    }
